...
POST https://api.zan.top/node/v1/polygon/mainnet/{apiKey}
...
## 也可以用其他支持的网络替换主网
curl https://api.zan.top/node/v1/polygon/mainnet/{apiKey} \
    -X POST \
...
返回当前网络的链 ID，其中 Polygon 主网的链 ID 为 137。这是 EIP-155 提案中引入的值，用于保护重放交易签名。
...
- fromBlock: int - 起始区块编号，用整数表示
- toBlock: int - 终止区块编号，用整数表示
...
blockNumber: string - 16 进制区块编号，或者字符串标签“latest”表示最新区块。
...
## 也可以用其他支持的网络替换主网
wscat -c wss://api.zan.top/node/ws/v1/polygon/mainnet/{apiKey}
>{"jsonrpc":"2.0","method":"bor_getSignersAtHash","params":["0x29fa73e3da83ddac98f527254fe37002e052725a88904bac14f03e919e1e2876"], "id":1}
```
...
返回当前的验证者列表。如需要了解更多关于 Polygon 验证者的信息，请参阅 [Bor 文档](https://wiki.polygon.technology/docs/pos/heimdall/modules/bor/)。
